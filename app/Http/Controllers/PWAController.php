...
class PWAController extends Controller
{
    ...
    {
        $manifest = [
            'name' => get_option('pwa_app_name', get_option('site_title', config('app.name'))),
            'short_name' => get_option('pwa_short_name', get_option('company_name', 'IntelliCash')),
            'description' => get_option('pwa_description', 'Progressive Web App for IntelliCash - Manage your finances efficiently'),
            'start_url' => '/',
            'display' => get_option('pwa_display_mode', 'standalone'),
            'background_color' => get_option('pwa_background_color', '#ffffff'),
            'theme_color' => get_option('pwa_theme_color', get_option('primary_color', '#007bff')),
            'orientation' => get_option('pwa_orientation', 'portrait-primary'),
            'scope' => '/',
    ...
    private function getShortcuts()
    {
        ...
        if (get_option('pwa_shortcut_dashboard', 1)) {
            $shortcuts[] = [
                'name' => 'Dashboard',
        ...
        if (get_option('pwa_shortcut_transactions', 1)) {
            $shortcuts[] = [
                'name' => 'Transactions',
        ...
        if (get_option('pwa_shortcut_profile', 1)) {
            $shortcuts[] = [
                'name' => 'Profile',
                'short_name' => 'Profile',
