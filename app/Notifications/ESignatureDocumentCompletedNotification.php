...
class ESignatureDocumentCompletedNotification extends Notification
{
    use Queueable;
    ...
    public function __construct(ESignatureDocument $document)
    {
        ...
        $this->template = EmailTemplate::where('slug', 'ESIGNATURE_DOCUMENT_COMPLETED')
        ...
        $this->replace['completedAt'] = $document->completed_at ? $document->completed_at->format('M d, Y \a\t g:i A') : now()->format('M d, Y \a\t g:i A');
        $this->replace['signerCount'] = $document->signatures()->where('status', 'signed')->count();
        $this->replace['documentUrl'] = route('esignature.esignature-documents.show', $document->id);
        $this->replace['downloadUrl'] = route('esignature.esignature-documents.download-signed', $document->id);
    ...
    public function toArray($notifiable)
    {
        ...
        return [
            ...
            'type' => 'esignature_document_completed'
        ];
    }
}
