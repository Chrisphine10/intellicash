...
class ESignatureReminderNotification extends Notification
{
    use Queueable;
    ...
    public function __construct(ESignatureDocument $document, ESignatureSignature $signature)
    {
        ...
        $this->template = EmailTemplate::where('slug', 'ESIGNATURE_REMINDER')
        ...
        $this->replace['signingUrl'] = $signature->getSignatureUrl();
        $this->replace['expiresAt'] = $signature->expires_at ? $signature->expires_at->format('M d, Y') : 'No expiration';
        $this->replace['daysRemaining'] = $signature->expires_at ? now()->diffInDays($signature->expires_at, false) : 'N/A';
