...
class ESignatureDocumentExpiredNotification extends Notification
{
    use Queueable;
    ...
    public function __construct(ESignatureDocument $document)
    {
        ...
        $this->template = EmailTemplate::where('slug', 'ESIGNATURE_DOCUMENT_EXPIRED')
        ...
        $this->replace['expiredAt'] = $document->expires_at ? $document->expires_at->format('M d, Y \a\t g:i A') : '';
        $this->replace['pendingSigners'] = $document->signatures()->where('status', 'pending')->count();
    ...
    public function toArray($notifiable)
    {
        ...
        return [
            ...
            'type' => 'esignature_document_expired'
        ];
    }
}
