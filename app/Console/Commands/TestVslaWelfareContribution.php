...
class TestVslaWelfareContribution extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'vsla:test-welfare-contribution {--tenant-id=1} {--member-id=1} {--amount=50} {--test : Test mode only}';
     ...
     * @var string
     */
    protected $description = 'Test VSLA welfare contribution functionality';
    ...
    public function handle()
    {
        ...
        $this->info('Testing VSLA Welfare Contribution functionality...');
        ...
        try {
            ...
            // Check VSLA Share Account (group account)
            $groupShareAccount = \App\Models\BankAccount::where('tenant_id', $tenantId)
                ->where('account_name', 'VSLA Share Account')
                ->first();

            if (!$groupShareAccount) {
                $this->error('VSLA Share Account (group) not found');
            ...
            $this->info("VSLA Share Account (group) found: ID {$groupShareAccount->id}");
            $this->info("Current Group Share Account Balance: " . number_format($groupShareAccount->current_balance ?? 0, 2));

            if ($testMode) {
                $this->info('✅ All checks passed - welfare contribution would be processed successfully');
                $this->info('Transaction flow:');
                $this->line("  1. Credit {$amount} to member's VSLA Share Account");
                $this->line("  2. Credit {$amount} to VSLA Share Account (group)");
                $this->line("  3. Create VSLA transaction record");
                return 0;
            }

            // Create test welfare contribution transaction
            $vslaTransaction = VslaTransaction::create([
                ...
                'amount' => $amount,
                'description' => 'Test welfare contribution - ' . now(),
                'status' => 'pending',
                'created_user_id' => 1,
            ]);
            ...
            $method = $reflection->getMethod('processWelfareContribution');
            $method->setAccessible(true);
            
            $method->invoke($controller, $vslaTransaction, $tenant);

            $this->info('✅ Welfare contribution processed successfully!');
            ...
            // Show updated group balance
            $groupShareAccount->refresh();
            $this->info("Updated Group Share Account Balance: " . number_format($groupShareAccount->current_balance ?? 0, 2));
