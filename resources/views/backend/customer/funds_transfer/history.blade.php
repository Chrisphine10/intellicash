    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-sm-flex align-items-center justify-content-between">
                ...
                <div>
                    <a class="btn btn-primary btn-xs" href="{{ route('funds_transfer.form') }}">
                        <i class="fas fa-plus mr-1"></i>{{ _lang('New Transfer') }}
                    </a>
                </div>
            </div>
            <div class="card-body">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($transfer->transfer_type === 'kcb_buni')
                                            <small class="text-muted">Account: {{ $transfer->beneficiary_account }}</small>
                                        @else
                                            <small class="text-muted">Mobile: +254{{ $transfer->beneficiary_mobile }}</small>
                                        @endif
                                    </td>
                                    <td class="text-right">
                ...
                @else
                    <div class="text-center py-4">
                        ...
                        <a href="{{ route('funds_transfer.form') }}" class="btn btn-primary">
                            <i class="fas fa-plus mr-1"></i>{{ _lang('Make Your First Transfer') }}
                        </a>
                    </div>
                @endif
            </div>
...
function viewTransferDetails(transferId) {
    ...
    $.ajax({
        url: '{{ route("funds_transfer.details", ":id") }}'.replace(':id', transferId),
        type: 'GET',
