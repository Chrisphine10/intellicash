			...
			<div class="card-body">
				@if($loanRepayments->count() > 0)
					<table class="table table-bordered data-table">
	                    ...
	                    <tr>
	                        ...
	                        <td>{{ $repayment->loan->borrower->name ?? 'N/A' }}</td>
	                        <td class="text-right">
	                            {{ decimalPlace($repayment['amount_to_pay'], currency($repayment->loan->currency->name ?? 'USD')) }}
	                        </td>
	                        <td class="text-right">
	                            {{ decimalPlace($repayment['principal_amount'], currency($repayment->loan->currency->name ?? 'USD')) }}
	                        </td>
	                        <td class="text-right">
	                            {{ decimalPlace($repayment['interest'], currency($repayment->loan->currency->name ?? 'USD')) }}
	                        </td>
	                        <td class="text-right">
	                            {{ decimalPlace($repayment['penalty'], currency($repayment->loan->currency->name ?? 'USD')) }}
	                        </td>
	                        <td class="text-right">
	                            {{ decimalPlace($repayment['balance'], currency($repayment->loan->currency->name ?? 'USD')) }}
	                        </td>
					...
					<!-- Pagination -->
					@if(method_exists($loanRepayments, 'links'))
				...
				@else
					<div class="alert alert-info text-center">
						<i class="fas fa-info-circle"></i> {{ _lang('No upcoming loan repayments found for the selected date range.') }}
					</div>
				@endif
			</div>
		</div>
